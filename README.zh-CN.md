...
4. **重新加载 pi** — 在 pi 中输入 `/reload`，然后使用 `/hermes hello` 测试。
...
```
you: /hermes -m grok-4.3 用3个要点解释量子纠缠

# pi 通知: → Hermes task abc123 started (grok-4.3): 用3个要点解释量子纠缠...

# ...几秒后，Hermes 响应。pi 自主审查结果。
# 如果 pi 发现错误，会通过 hermes-review 发送反馈（最多 3 轮）。
# 如果一切正常，pi 静默综合答案。
```
...
| `/reload` 后扩展未加载 | 文件位置错误 | 检查 `~/.pi/agent/extensions/hermes.ts` 是否存在 |
| 任务超时失败 | Hermes CLI 挂起或模型不可用 | 在终端中直接运行 `hermes chat -q "test" -m grok-4.3` |
| `hermes-review` 显示 "Task not found" | taskId 错误或任务已过期 | 使用 `/hermes --status` 查找正确的任务 ID |
